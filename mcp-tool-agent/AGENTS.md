...
Recommended workflow: **`api-integration`**.

## What this repo is

A **self-contained MCP/Tool agent package**: the agent definition (`agent.md`),
...
recommended for MCP/tool engineering — plus a full live inventory of the MCP
...
2. Load the workflow `workflows/api-integration.md`.
...
5. Fan out to `subagents/` (tool-scout → mcp-builder → verifier →
   contract-writer → gateway-planner) and merge.
...
| Path | Contents |
|---|---|
| `agent.md` | The mcp/tool agent definition (main deliverable). |
| `subagents/` | Tool-scout / mcp-builder / verifier / contract-writer / gateway-planner. |
| `skills/` | 16 verbatim `SKILL.md` copies. See `skills-index.md`. |
| `workflows/` | 4 verbatim workflow `.md` files. See `workflows-index.md`. |
| `mcpservers/` | JSON entries: recommended gateway/tool MCPs + installed on this machine. |
