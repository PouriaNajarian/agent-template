# AGENTS.md — trading-strategy-agent
...
Recommended workflows: **`fw-model-development`** + **`fw-model-evaluation`**.

## What this repo is

A **self-contained trading-strategy agent package**: the agent definition
...
recommended for strategy design — plus a full live inventory of the MCP
servers and functions installed on this machine (`mcp-inventory/`).
...
2. Load the workflows `workflows/fw-model-development.md` +
   `workflows/fw-model-evaluation.md`.
...
5. Fan out to `subagents/` (edge-hunter → signal-designer → risk-designer →
   test-plan-writer → spec-writer) and merge.
6. Emit the strategy specification format defined in `agent.md`.

## Layout

| Path | Contents |
|---|---|
| `agent.md` | The trading-strategy agent definition (main deliverable). |
| `subagents/` | Edge-hunter / signal-designer / risk-designer / test-plan-writer / spec-writer. |
| `skills/` | 11 verbatim `SKILL.md` copies. See `skills-index.md`. |
| `workflows/` | 4 verbatim workflow `.md` files. See `workflows-index.md`. |
| `mcpservers/` | JSON entries: recommended strategy MCPs + installed on this machine. |
