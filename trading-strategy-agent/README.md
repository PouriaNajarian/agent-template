...
A **trading strategy design AI agent** for trading/investment systems, built
...
```
trading-strategy-agent/
├── agent.md              # the trading strategy agent (start here)
...
├── mcpservers/           # recommended strategy MCP-server entries
...
- **Workflows**: `fw-model-development`, `fw-model-evaluation`
- **Top skills**: `quant-analyst`, `trading-ledger`, `risk-manager`,
  `price-psychology-strategist`, `ml-model-evaluation`
- **MCP servers**: `alpaca`, `crashtestyourstrategy`, `botspot`, `arrow-algo`,
  `alphaassay`, `alforge-labs-alpha-forge-mcp`, `keel-trade-keel-trade`
- **Tools**: Backtrader, VectorBT, QuantConnect LEAN, pandas-ta
...
1. Open `agent.md`.
2. Load `workflows/fw-model-development.md`.
3. Design a strategy:
   ```
   "design a mean-reversion strategy for BTCUSDT on H1 with defined risk limits"
   ```
4. Produce the spec using the format in `agent.md`; hand to the backtest agent.
