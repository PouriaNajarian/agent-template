# AGENTS.md — project-management-agent
...
Recommended workflows: **`fw-role-separated-agents`** + **`fw-agentic-coding-loop`**.

## What this repo is

A **self-contained project-management agent package**: the agent definition
...
recommended for project management — plus a full live inventory of the MCP
...
2. Load the workflows `workflows/fw-role-separated-agents.md` +
   `workflows/fw-agentic-coding-loop.md`.
...
5. Fan out to `subagents/` (planner → ticket-writer → scheduler →
   risk-tracker → status-reporter) and merge.
...
| Path | Contents |
|---|---|
| `agent.md` | The project-management agent definition (main deliverable). |
| `subagents/` | Planner / ticket-writer / scheduler / risk-tracker / status-reporter. |
| `skills/` | 20 verbatim `SKILL.md` copies. See `skills-index.md`. |
| `workflows/` | 8 verbatim workflow `.md` files. See `workflows-index.md`. |
| `mcpservers/` | JSON entries: recommended PM MCPs + installed on this machine. |
