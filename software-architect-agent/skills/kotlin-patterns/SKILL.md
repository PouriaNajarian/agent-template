...
description: Patrones idiomÃ¡ticos de Kotlin, buenas prÃ¡cticas y convenciones para construir aplicaciones Kotlin robustas, eficientes y mantenibles con coroutines, null safety y builders de DSL.
---

# Patrones de Desarrollo Kotlin

Patrones idiomÃ¡ticos de Kotlin y buenas prÃ¡cticas para construir aplicaciones robustas, eficientes y mantenibles.

## CuÃ¡ndo Usar

- Escribir nuevo cÃ³digo Kotlin
- Revisar cÃ³digo Kotlin
- Refactorizar cÃ³digo Kotlin existente
- DiseÃ±ar mÃ³dulos o librerÃ­as Kotlin
- Configurar builds con Gradle Kotlin DSL

## CÃ³mo Funciona

Este skill aplica convenciones idiomÃ¡ticas de Kotlin en siete Ã¡reas clave: null safety usando el sistema de tipos y operadores de llamada segura, inmutabilidad mediante `val` y `copy()` en data classes, clases selladas e interfaces para jerarquÃ­as de tipos exhaustivas, concurrencia estructurada con coroutines y `Flow`, funciones de extensiÃ³n para agregar comportamiento sin herencia, builders de DSL type-safe usando `@DslMarker` y receptores lambda, y Gradle Kotlin DSL para configuraciÃ³n de build.
...
El sistema de tipos de Kotlin distingue tipos nullable de no-nullable. Aprovecharlo al mÃ¡ximo.
...
### 3. Cuerpos de ExpresiÃ³n y Funciones de Una Sola ExpresiÃ³n

Usar cuerpos de expresiÃ³n para funciones concisas y legibles.

```kotlin
// Bien: Cuerpo de expresiÃ³n
...
// Bien: Value class para type safety (cero overhead en tiempo de ejecuciÃ³n)
@JvmInline
...
### Modelar JerarquÃ­as Restringidas
...
// with: Forma no-extensiÃ³n de run
...
```kotlin
// Bien: Extensiones especÃ­ficas del dominio
...
```kotlin
// Bien: Flow frÃ­o con manejo de errores adecuado
...
### CancelaciÃ³n y Limpieza

```kotlin
// Bien: Respetar la cancelaciÃ³n
suspend fun processItems(items: List<Item>) {
    items.forEach { item ->
        ensureActive() // Verificar cancelaciÃ³n antes del trabajo costoso
        processItem(item)
    }
}
...
### DelegaciÃ³n de Interfaces

```kotlin
// Bien: Delegar implementaciÃ³n de interfaz
...
## Secuencias para EvaluaciÃ³n Diferida

```kotlin
// Bien: Usar secuencias para colecciones grandes con mÃºltiples operaciones
...
### ConfiguraciÃ³n de build.gradle.kts

```kotlin
// Verificar las Ãºltimas versiones: https://kotlinlang.org/docs/releases.html
plugins {
...
### Procesamiento IdiomÃ¡tico de Colecciones
...
## Referencia RÃ¡pida: Modismos de Kotlin

| Modismo | DescripciÃ³n |
|---------|-------------|
| `val` sobre `var` | Preferir variables inmutables |
| `data class` | Para objetos de valor con equals/hashCode/copy |
| `sealed class/interface` | Para jerarquÃ­as de tipos restringidas |
| `value class` | Para wrappers type-safe con cero overhead |
| `when` expresiÃ³n | Pattern matching exhaustivo |
| Llamada segura `?.` | Acceso a miembros null-safe |
| Elvis `?:` | Valor por defecto para nullables |
| `let`/`apply`/`also`/`run`/`with` | Funciones de scope para cÃ³digo limpio |
| Funciones de extensiÃ³n | Agregar comportamiento sin herencia |
| `copy()` | Actualizaciones inmutables en data classes |
| `require`/`check` | Aserciones de precondiciones |
| Coroutine `async`/`await` | EjecuciÃ³n concurrente estructurada |
| `Flow` | Streams reactivos frÃ­os |
| `sequence` | EvaluaciÃ³n diferida |
| DelegaciÃ³n `by` | Reutilizar implementaciÃ³n sin herencia |
...
**Recuerda**: El cÃ³digo Kotlin debe ser conciso pero legible. Aprovecha el sistema de tipos para seguridad, prefiere la inmutabilidad y usa coroutines para concurrencia. Ante la duda, deja que el compilador te ayude.
