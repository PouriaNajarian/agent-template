...
1. **Clean Architecture** (default) â layers (Domain, Application, Interface, Infrastructure), dependency inversion, command/query separation
2. **Hexagonal / Ports & Adapters** â core domain surrounded by ports, adapters on the outside
3. **Modular Monolith** â vertical slices, each module owns its own layers
4. **Custom / Define from scratch** â you describe the layers and rules"

**Branching:**

- **Option 1** â proceed to the clean architecture flow below (existing interview). Template: `./assets/template-clean-arch.md`. Output: `.lattice/standards/architecture.md`. Config key: `paths.architecture`. No `architecture_mode` key needed (defaults to `clean`).
- **Options 2â4** â proceed to the generic architecture flow. Template: `./assets/template-generic.md`. Output: `.lattice/standards/architecture.md`. Config key: `paths.architecture`. Additionally, set `architecture_mode: custom` in `.lattice/config.yaml`.

The rest of this document describes the **clean architecture flow** (Option 1). For the **generic flow** (Options 2â4), read `./assets/template-generic.md` and follow its `<!-- INTERVIEW GUIDANCE: -->` comments. The facilitation approach, conversation style, output assembly, and document quality checks below apply to both flows â substitute the appropriate template, output path, and config key.

## What This Produces

**For clean architecture (Option 1):**

- **Output**: `.lattice/standards/architecture.md` (or custom path from `.lattice/config.yaml` â `paths.architecture`)
- **Two modes**:
...
**For other styles (Options 2â4):**

- **Output**: `.lattice/standards/architecture.md` (or custom path from `.lattice/config.yaml` â `paths.architecture`)
- **Mode**: Always `override` â there are no embedded defaults to overlay onto for non-clean-architecture styles
- **Config key**: `paths.architecture` in `.lattice/config.yaml`
...
1. Read `.lattice/config.yaml` â check `paths.architecture`.
...
1. **Customize specific sections** (overlay) â Keep the defaults and change only what differs for your project. This produces a slim document. Most teams choose this.
2. **Define everything from scratch** (override) â Walk through all sections and produce a comprehensive standalone document.
3. **Add project-specific sections only** (overlay with additions) â Keep all defaults as-is and add new sections for your team's specific rules.
...
3. If the user says it matches â skip it (section will NOT appear in the output).
4. If the user wants changes â dive into that section, discuss the specifics, record the changes.
...
- **"I agree with everything"** â No custom document needed. Tell the user: "The embedded defaults are already active and match your preferences. No custom document is needed â the architecture atom will use the clean-architecture defaults automatically."
- **"I agree except one section"** â Overlay mode, interview that one section only.
- **"We use CQRS"** â Overlay Â§3.2 + Â§4 (they are coupled â CQRS changes the service pattern which changes both flows).
- **"We don't use Providers"** â Overlay Â§3.4 + Â§4.2 + Â§4.3 + Â§6 (Provider removal ripples through query flow and validation).
- **"We have extra layers"** â Overlay Â§1 + Â§2 + Â§3 (new layers need responsibilities, dependency placement, and per-layer rules).
...
|-------------|---------|-----|
| Â§1 â Layer names | All sections | Names must be consistent everywhere |
| Â§1 â Extra layers | Â§2 (diagram), Â§3 (per-layer rules) | New layers need dependency placement and rules |
| Â§3.2 â Service pattern (unified vs CQRS) | Â§4.1, Â§4.2 | CQRS uses separate handlers instead of unified service |
| Â§3.4 â Provider pattern (yes/no) | Â§4.2, Â§4.3, Â§6 | No Provider â reads go through Repository; comparison table and checklist change |
...
4. **No** â Dive into the section details using the template guidance. Produce the user's version.
...
3. **As-is** â Include the default content in the output unchanged.
...
5. Each section must be self-contained â it is a complete replacement of that section in defaults. Do not write diffs or partial sections.
6. Section headings must match `clean-architecture-defaults.md` exactly (the atom matches sections by heading)
7. New sections (Â§7+) are included after the default sections
...
This is the same for all styles â both clean architecture customizations and other styles write to `paths.architecture`.
...
- [ ] Only changed/added sections are included â unchanged sections are omitted

### Override mode checks

- [ ] Every section from the template is present (Â§1 through Â§6, plus any new sections)
- [ ] Layer names are consistent throughout all sections
- [ ] Dependency diagram (Â§2) matches the layer table (Â§1)
- [ ] Code examples use pseudocode (language-agnostic, same style as defaults.md)
- [ ] Validation checklist (Â§6) is consistent with the rules defined in Â§3 and Â§4
- [ ] No `<!-- INTERVIEW GUIDANCE: -->` comments remain
- [ ] Frontmatter has `mode: override`
- [ ] Document is readable as a standalone specification

### Generic flow checks (Options 2â4)

- [ ] Document has `mode: override` in frontmatter
- [ ] Sections Â§1 through Â§7 are present (Â§8 Ambiguity Signals is optional, plus any new sections)
- [ ] Layer names are consistent throughout all sections
- [ ] Dependency diagram (Â§2) matches the layer table (Â§1)
- [ ] Â§6 (Validation Checklist) contains at least 3 concrete, verifiable checks
- [ ] Â§7 (Anti-Patterns) contains at least 3 anti-patterns with symptom and fix
- [ ] No `<!-- INTERVIEW GUIDANCE: -->` comments remain
- [ ] Document is readable as a standalone specification
