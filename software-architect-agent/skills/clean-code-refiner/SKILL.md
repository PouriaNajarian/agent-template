...
- **"We use shorter functions"** -> Overlay Â§2 (thresholds change from ~20 to whatever the team prefers).
- **"We use Result types instead of exceptions"** -> Overlay Â§8 (error handling patterns change fundamentally).
- **"We're a functional team -- no classes"** -> Overlay Â§1 (remove class cohesion guidance), Â§5 (parameter patterns for functional style), Â§9 (functional patterns emphasis).
- **"We want stricter complexity limits"** -> Overlay Â§3 (adjust thresholds, e.g., max complexity 5 instead of 10).
- **"We have language-specific idioms"** -> Overlay with additions, e.g., Â§11 Go-specific patterns, Â§12 Python-specific patterns.
...
|-------------|---------|-----|
| Â§1 -- SRP scope (classes vs functions-only) | Â§2 (extraction targets), Â§10 (checklist) | Functional codebases extract to functions only; class-based codebases also extract to classes |
| Â§2 -- Function size thresholds | Â§3 (complexity thresholds), Â§10 (checklist) | Shorter functions imply lower complexity budgets |
| Â§3 -- Complexity thresholds | Â§2 (function size) | Lower complexity limits may require stricter function size |
| Â§4 -- Naming conventions | Â§7 (comment necessity) | Better naming reduces the need for "what" comments |
| Â§5 -- Parameter design | Â§1 (SRP signals) | Long parameter lists often signal SRP violations |
| Â§8 -- Error handling strategy | Â§9 (testability patterns) | Result types vs exceptions change how error paths are tested |
...
7. New sections (Â§11+) are included after the default sections
...
- [ ] Every section from the template is present (Â§1 through Â§10, plus any new sections)
...
- [ ] Validation checklist (Â§10) is consistent with the principles defined in Â§1 through Â§9
- [ ] No `<!-- INTERVIEW GUIDANCE: -->` comments remain
