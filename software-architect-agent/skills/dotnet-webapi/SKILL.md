...
**Date and time values â use `DateTimeOffset`:** When a DTO includes a date or
...
conversions, and serializes to ISO 8601 with offset information in JSON â which
is what API consumers expect.

Reference: https://learn.microsoft.com/en-us/dotnet/api/system.datetimeoffset
**JSON serialization options â preserve existing behavior by default:** For
...
```
**Enum properties â serialize as strings by default:** Unless the user
...
**Response DTOs** â use positional sealed records for concise, immutable output:
...
**Request DTOs** â use sealed records with `init` properties so data annotations
work naturally:
...
**Minimal API validation â register explicitly:** Data-annotation validation
...
**Minimal API return types â prefer `TypedResults`:**
...
**Fallback â `Results` factory:** If a handler has many conditional branches
...
// Minimal API example â TypedResults with explicit return type (recommended)
...
This is all that is needed â no additional packages required.
    ...
    : IExceptionHandler
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await httpContext.Response.WriteAsJsonAsync(new ProblemDetails
        {
            Status = statusCode,
            Title = title,
            // Do not use exception.Message here â it may leak sensitive internal details.
...
Always define an interface for every service â this enables unit testing with
...
| `TypedResults` in ternary without explicit return type | `Ok<T>` and `NotFound` have no common base â annotate with `Task<Results<Ok<T>, NotFound>>` or fall back to `Results` factory. |
...
| Using `DateTime` for date/time properties | Use `DateTimeOffset` instead â it preserves UTC offset, avoids timezone ambiguity, and serializes correctly in JSON. |
...
- [OpenAPI from XML comments](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/openapi/openapi-comments) â how XML doc comments flow into the OpenAPI spec
...
- [DateTimeOffset](https://learn.microsoft.com/en-us/dotnet/api/system.datetimeoffset) â preferred type for date/time values in APIs
