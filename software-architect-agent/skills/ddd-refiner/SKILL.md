...
- **Output**: `.lattice/standards/ddd-principles.md` (or custom path from `.lattice/config.yaml` â `paths.ddd_principles`)
- **Two modes**:
...
- **"I agree with everything"** â No custom document needed. Tell the user: "The embedded defaults are already active and match your preferences. No custom document is needed -- the domain-driven-design atom will use the defaults automatically."
- **"I agree except one section"** â Overlay mode, interview that one section only.
- **"We have anemic entities and want to fix that"** â Overlay Â§2 (entity patterns, anemic anti-pattern is inline) + Â§9 (entity checks).
- **"We don't use domain events yet"** â Overlay Â§5 (domain events) with simplified approach or removal note. Also check Â§1 (cross-aggregate coordination, anti-pattern is inline).
- **"Our aggregates are too big"** â Overlay Â§1 (aggregate design, god aggregate anti-pattern is inline) + Â§8 (decomposition guide).
- **"We want to add a ubiquitous language glossary"** â Overlay with new Â§10 only.
...
|-------------|---------|-----|
| Â§1 â Aggregate boundaries | Â§6 (repositories), Â§5 (events), Â§8 (decomposition) | One repo per aggregate root; events for cross-aggregate coordination |
| Â§1 â Sizing thresholds | Â§8 (decomposition triggers) | Custom thresholds change decomposition warning signals |
| Â§2 â Entity identity strategy | Â§3 (typed ID value objects), Â§6 (repository signatures) | Typed IDs must be value objects; repository findById uses typed IDs |
| Â§3 â Value object catalog | Â§2 (entity fields) | New value objects appear in entity definitions |
| Â§5 â Event patterns | Â§1 (cross-aggregate coordination) | Events are the mechanism for cross-aggregate consistency |
| Â§6 â Repository patterns | Â§1 (aggregate root identification) | Only roots get repositories |
...
7. New sections (Â§10+) are included after the default sections
...
- [ ] Every section from the template is present (Â§1 through Â§9, plus any new sections)
...
- [ ] Validation checklist (Â§9) is consistent with the rules defined in Â§1 through Â§7
