...
description: Facilitate a structured conversation to define requirement standards for a project â epic and feature definitions, scenario structure, AC format, priority notation, status workflow, and naming conventions. Produces a formal requirement-standards.md that the requirement-quality atom reads via config resolution, customising its embedded defaults for the team's product process. Use when setting up a new project, defining product standards, or when the user says 'set up requirement standards', 'defi
---
...
- **Output**: `.lattice/standards/requirement-standards.md` (or custom path from `.lattice/config.yaml` â `paths.requirement_standards`)
- **Two modes**:
...
- **Default mode**: Overlay â produces only what the team wants to change
- **Config key**: `paths.requirement_standards` in `.lattice/config.yaml`
- **Consumed by**: `requirement-quality` atom (via config resolution) â `requirement-forge` molecule (composes the atom)
...
The standards produced here answer: what is an epic, what is a feature, what is a scenario, how are ACs written, how are features named and prioritized. These are the rules the `requirement-quality` atom enforces â the molecule composes the atom and inherits those rules automatically.

## Before You Begin

### Check for an existing standards document

1. Read `.lattice/config.yaml` â check `paths.requirement_standards`.
...
1. "Does your team already have a way of writing requirements â any existing PRDs, Confluence templates, or Jira conventions I should be aware of?"
2. "Is there a specific product domain or terminology I should know before we define the standards?"

These two questions are the only free-form listening before the structured interview begins. Synthesize what you hear and carry it forward â do not ask follow-up questions at this stage.
...
1. **Customize specific sections** (overlay) â Keep the built-in defaults and change only what differs for your project. This produces a slim document. Most teams choose this.
2. **Define everything from scratch** (override) â Walk through all sections and produce a comprehensive standalone document.
3. **Add project-specific sections only** (overlay with additions) â Keep all defaults as-is and add new sections, such as domain terminology or custom status workflows.
...
Map the choice:
- Options 1 and 3 â `mode: overlay`
- Option 2 â `mode: override`
...
- **Defaults-first.** For each section, briefly summarize the default, then ask if it matches. Do not read defaults verbatim â summarize key points and ask.
- **Propose, don't just ask.** When the user's answer is ambiguous, propose the most reasonable interpretation and ask them to confirm or correct. "It sounds like you want MoSCoW priorities â so 'Must', 'Should', 'Could', 'Won't'. Is that right?"
...
- **Challenge weak definitions.** If the user defines a "feature" so broadly it would encompass an entire epic, push back: "That scope sounds like an epic â a feature should be independently designable in one design-blueprint session. Can we tighten the definition?"

### For overlay mode

This should be fast. Many sections will be "keep as-is."

1. Present each section's default in 2â3 sentences.
2. Ask: "Does this match your project, or would you like to change it?"
3. If matches â skip it (section will NOT appear in the output).
4. If changes wanted â discuss specifics, record the changes.
5. After all sections, ask: "Anything to add that isn't covered â domain-specific terminology, custom fields, team conventions?"
...
- **"I agree with everything"** â No custom document needed. "The embedded defaults are already active. No custom document is needed â requirement-forge will use the defaults automatically."
- **"We use MoSCoW priorities"** â Overlay Â§6 only.
- **"We call them 'use cases' not 'scenarios'"** â Overlay Â§4 (rename + update any description that references "scenario").
- **"We have a longer status workflow"** â Overlay Â§7.
- **"We have domain-specific terms that should be consistent"** â Overlay with additions â add Â§10 Domain Terminology.
- **"Our features tend to be larger"** â Overlay Â§2 and Â§4 â adjust the size signals and max scenario count.
...
|---|---|---|
| Â§2 â Feature size definition | Â§4 scenario count | Larger features tolerate more scenarios; tighter features need a lower cap |
| Â§4 â Scenario nomenclature | Â§8 naming conventions | If "scenario" is renamed, naming conventions must use the new term |
| Â§4 â Max scenarios per feature | Â§2 feature definition | These two must be consistent â the split signal in Â§2 should align with the cap in Â§4 |
| Â§5 â AC format | Â§4 scenario structure | AC format determines what each scenario's criteria look like |
| Â§6 â Priority notation | feature file frontmatter | Priority field format used in every generated feature file |
| Â§7 â Status workflow | feature file frontmatter | Status field used in every generated feature file |
| Â§8 â Naming conventions | all file generation | Feature file names and display names generated by the molecule |

When a dependency is triggered, inform the user: "Since you changed [X], we should also review [Y] â it's affected by that decision."

### Overlay-specific section flow

For each of the 9 default sections:

1. Summarize the section's key points in 2â3 sentences.
2. Ask: "Does this match your project?"
3. **Yes** â Move to the next section. This section will not appear in the output.
4. **No** â Dive into the details using the template guidance. Produce the user's version.
...
3. **As-is** â Include the default content unchanged.
4. **Modify** â Discuss changes, produce the modified version.
...
4. Only the sections the user changed or added â each must be self-contained and complete
5. Section headings must match `template.md` exactly (the molecule matches sections by heading)
6. New sections (Â§10+) included after the default sections
...
- [ ] Â§2 feature size signal is consistent with Â§4 max scenario count
- [ ] Â§4 scenario nomenclature is consistent with Â§8 naming conventions
- [ ] Â§5 AC format is consistent with how Â§4 describes scenario criteria
- [ ] Â§6 priority values and Â§7 status values match the frontmatter field descriptions in Â§8
- [ ] No `<!-- INTERVIEW GUIDANCE: -->` comments remain
