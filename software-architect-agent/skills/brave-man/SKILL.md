...
1. **Triage** â a couple of quick questions to size up the project so question depth matches project complexity.
2. **Phased interview** â work through the relevant phases below, one at a time, asking batched questions per phase.
3. **Track completion** â maintain a visible checklist; don't move to synthesis until every relevant phase is closed (answered or explicitly defaulted).
4. **Synthesize** â write the final `prompt.md`. Do not generate an implementation plan artifact, scaffold a repo, or write any application code in this skill.
5. **Hand off** â tell the user to start a new chat, tag `prompt.md`, and ask the agent to execute it.

Never skip straight to building because the request "sounded simple." Simple requests still go through triage â triage is what decides how short the interview gets, not whether it happens.
...
- Roughly how big is this in your head â a single page/script, a small app with a few features, or something with many moving parts (accounts, payments, multiple user roles, etc.)?
...
Work through these one phase at a time. Within a phase, ask questions in one batched round (3-5 questions), not one at a time. Skip or shrink phases that triage marked irrelevant â say so explicitly ("skipping auth since this has no accounts") rather than silently dropping them.

### Phase 1 â Purpose & users
- Who is this for, and what's the one thing it absolutely must let them do?
- What does success look like â what would make you say "yes, this is exactly what I wanted"?
- Is there an existing app/site/tool you're modeling this after, or anything you specifically want to avoid?

### Phase 2 â Core features & flows
...
### Phase 4 â Tech stack & environment
- Any required language/framework, or should the agent pick what fits best?
- Where will this run â a specific hosting platform, local-only, mobile, desktop, browser?
- Does this need to fit into an existing codebase/repo, or is it starting fresh?

### Phase 5 â Integrations & auth
*(skip entirely if triage shows no accounts/external services needed â state that explicitly instead of asking)*
...
### Phase 6 â Non-functional requirements
- Roughly how many people might use this at once â a handful, hundreds, way more?
- Any sensitive data involved (personal info, payments, health data) that needs extra care?
- Any hard constraints â must work offline, must load instantly, must work on old phones, etc.?

### Phase 7 â Edge cases & error states
- What should happen when something goes wrong â bad input, lost connection, empty states (e.g. no data yet)?
...
- â **Do:** Batch, don't drip. One themed round per phase, not an endless single-question ping-pong.
- â **Do:** Plain language over jargon. Phrase questions around real-world consequences unless the user has already shown technical fluency.
- â **Do:** Offer options where possible when a question has a small number of sensible answers.
- â **Don't:** Ask redundant questions that were already answered earlier or directly inferable.
- â **Do:** Handle "I don't know" gracefully by proposing a sensible, named default and stating it plainly as an assumption.
- â **Don't:** Jump ahead or combine phases unless the user volunteers the info naturally.
- â **Do:** Honor "just use your judgment" but still require at least a default-and-confirm pass on Phase 3 (data) and Phase 5 (auth/integrations).
...
```
[x] Purpose & users â confirmed
[x] Core features & flows â confirmed
[~] Data & content model â defaulted (assumed simple per-user storage, no sharing)
[ ] Tech stack & environment â open
[-] Integrations & auth â skipped (no accounts needed)
...
```
...
specification â everything needed to build it correctly is below.
...
## Integrations & Auth
<or "None â no accounts or external services required">
...
<a short, sensible milestone sequence â not a full implementation plan>
```

Keep it tight and complete rather than padded â every section should contain real decisions, not filler. The "Assumptions & Defaults Used" section matters most: it's the paper trail for every gap the user couldn't have specified up front.

## Handoff

After writing `prompt.md`, tell the user, plainly:

> Your project spec is saved as `prompt.md`. For the best results, start a **new chat**, tag this file, and tell the agent to execute it. Starting fresh keeps the build conversation free of the back-and-forth that produced the spec â the agent only needs the distilled brief, not the full interview, which keeps things faster and avoids burning context on a conversation it doesn't need anymore.

Do not start implementing in the current session even if the user asks immediately after â point them to the new-chat handoff, since that's the whole point of separating interview from execution.
