...
âââ Severity: critical or high
â   âââ Is the vulnerable code reachable in runtime, build, test, or deployment paths?
â   â   âââ YES --> Fix immediately (update, patch, or replace the dependency)
â   â   âââ NO (confirmed unused across those paths) --> Fix soon, but not a blocker
â   âââ Is a fix available?
â       âââ YES --> Update to the patched version
â       âââ NO --> Check for workarounds, consider replacing the dependency, or add to allowlist with a review date
âââ Severity: moderate
â   âââ Reachable in production? --> Fix in the next release cycle
...
```
.env files:
  âââ .env.example  â Committed (template with placeholder values)
  âââ .env          â NOT committed (contains real secrets)
