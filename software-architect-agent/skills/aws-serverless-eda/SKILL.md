...
Always verify AWS facts using MCP tools (`mcp__aws-mcp__*` or `mcp__*awsdocs*__*`) before answering. The `aws-mcp-setup` dependency is auto-loaded â if MCP tools are unavailable, guide the user through that skill's setup flow.
...
> - AWS Serverless MCP â SAM CLI lifecycle (init, deploy, local test)
> - AWS Lambda Tool MCP â Direct Lambda invocation
> - AWS Step Functions MCP â Workflow orchestration
> - Amazon SNS/SQS MCP â Messaging and queue management
...
```typescript
// â GOOD - Single purpose, focused function
...
```typescript
// â BAD - Lambda function chaining
...
// â GOOD - Step Functions orchestration
