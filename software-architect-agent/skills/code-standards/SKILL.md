...
2. **Plan if non-trivial** (3+ steps or an architectural choice): short plan in `tasks/todo.md` â
   files touched, tests to add, risks. Going sideways â stop and re-plan.
...
5. **Test:** new behavior gets a test that fails without the change; security gates get
   negative-assertion tests (wrong token â denied, state unchanged, no data leak).
...
8. **Close out:** update docs in the same change; append to a lessons file if you were
   corrected or surprised; write a conventional commit message (`feat|fix|docs(scope): â¦`).

## Tripwires â stop and re-check the project's guidelines when you're about toâ¦

- change a public or shared interface (API, wire format, schema) â check every caller; prefer additive changes
- add a dependency â weigh transitive weight and licenses, and whether the stdlib already covers it
- touch auth, tokens, or permissions â audit every writer and reader path, add negative tests
- write a secret to disk â use the platform's atomic create-private API, never write-then-chmod
- hold a lock across an `await`/blocking call, or read env inside logic â extract a pure decision function
