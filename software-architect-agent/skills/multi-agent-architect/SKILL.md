...
```
multi_agent_system/
âââ agents/          # One file per agent role
âââ tools/           # Tool definitions and wrappers
âââ memory/          # Redis, VectorDB, LangChain memory helpers
âââ prompts/         # Prompt templates (one per agent)
âââ workflows/       # High-level orchestration logic
âââ graphs/          # LangGraph state + compiled graph definitions
âââ api/             # FastAPI routes (optional)
âââ configs/         # Config loader â no secrets in code
âââ tests/           # Unit + integration tests per agent
âââ main.py
```
...
- â One agent = one responsibility â never combine planning + coding + testing in one node
- â Use `TypedDict` for all state schemas â enables type checking and graph validation
- â Bind only the tools each agent needs â reduces hallucinated tool calls
- â Always add a `step_count` guard to prevent infinite routing loops
- â Use `async`/`await` throughout â LangGraph supports async natively
- â Store all secrets in environment variables loaded via `os.getenv()`
- â Set TTLs on all Redis keys scoped to `session_id`
- â Log at every node entry and tool call for observability
- â Validate supervisor routing output against an allowlist of agent names
- â Don't hardcode API keys, model names, or Redis URLs
- â Don't share tool lists across agents that don't need them
- â Don't skip error handling â tool failures and empty LLM responses are common
- â Don't trust unvalidated LLM routing decisions â always check against an allowlist
...
- Never expose API keys in generated code. All secrets must use environment variables:
  ```python
  OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")   # â correct
  leaked_openai_token = "[redacted API key]"       # â never do this
  ```
