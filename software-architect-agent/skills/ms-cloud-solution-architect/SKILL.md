---
name: ms-cloud-solution-architect
description: >-
...
| **Web-Queue-Worker** | Web frontend â message queue â backend worker | Moderate-complexity apps with long-running tasks | App Service, Service Bus, Functions |
...
- **Domain complexity** â Microservices (high), N-tier (low-medium)
- **Team autonomy** â Microservices (independent teams), N-tier (single team)
- **Data volume** â Big data (TB+), others (GB)
- **Latency requirements** â Event-driven (real-time), Web-Queue-Worker (tolerant)
...
For each technology area, evaluate: **requirements â constraints â tradeoffs â select**.
...
| **Monolithic Persistence** | Single data store for all data types | Polyglot persistence â right store for each workload |
...
- **Identity & access** â Microsoft Entra ID, managed identity, RBAC
- **Monitoring** â Application Insights, Azure Monitor, Log Analytics
- **Security** â Network segmentation, encryption at rest/in transit, Key Vault
- **CI/CD** â GitHub Actions, Azure DevOps Pipelines, infrastructure as code
...
- [Design Patterns Reference](./references/design-patterns.md) â Detailed pattern implementations
- [Technology Choices Reference](./references/technology-choices.md) â Decision trees for Azure services
- [Best Practices Reference](./references/best-practices.md) â Implementation guidance
- [Mission-Critical Reference](./references/mission-critical.md) â High-availability design

---

## Source

Content derived from the [Azure Architecture Center](https://learn.microsoft.com/en-us/azure/architecture/) â Microsoft's official guidance for cloud solution architecture on Azure. Covers design principles, architecture styles, cloud design patterns, technology choices, best practices, performance antipatterns, mission-critical design, and the Well-Architected Framework.
