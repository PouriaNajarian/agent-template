...
- â Threading, locks, synchronization primitives
- â Random number generation (`random()`)
- â Global state or static variables
- â System time (`datetime.now()`)
- â Direct file I/O or network calls
- â Non-deterministic libraries

**Allowed in Workflows**:

- â `workflow.now()` (deterministic time)
- â `workflow.random()` (deterministic random)
- â Pure functions and calculations
- â Calling activities (non-deterministic operations)
