# AGENTS.md — market-researcher-agent
...
Recommended workflow: **`fw-research-optimize-execute`**.

## What this repo is

A **self-contained market-researcher agent package**: the agent definition
...
recommended for market research — plus a full live inventory of the MCP servers
...
5. For multi-part questions, fan out to `subagents/` (sector-scout →
   news-hunter → data-gatherer → synthesis-writer → citation-editor) and merge.
...
| Path | Contents |
|---|---|
| `agent.md` | The market-researcher agent definition (main deliverable). |
| `subagents/` | Sector-scout / news-hunter / data-gatherer / synthesis-writer / citation-editor. |
| `skills/` | 22 verbatim `SKILL.md` copies. See `skills-index.md`. |
| `workflows/` | 7 verbatim workflow `.md` files. See `workflows-index.md`. |
| `mcpservers/` | JSON entries: recommended research/finance MCPs + installed on this machine. |
| `agents/` | JSON entries for recommended agent platforms (claude-code, dify, crewai, langflow...). |
...
2. **Report back** — after each research step, update the notes/graph, query
   the graph, and pass only the graph summary + this advice as context to the
   LLM (saves tokens).
