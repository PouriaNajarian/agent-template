...
Recommended workflow: **`fw-model-evaluation`**.

## What this repo is

A **self-contained backtest agent package**: the agent definition (`agent.md`),
...
recommended for backtesting — plus a full live inventory of the MCP servers
...
2. Load the workflow `workflows/fw-model-evaluation.md`.
3. Load the specific skills the backtest needs from `skills/`
   (see `skills-index.md`).
4. Use the MCP tools listed in `agent.md`; confirm they exist via
   `mcp-inventory/system-mcps.md` / `mcp-inventory/mcp-functions.md`.
5. Fan out to `subagents/` (spec-interpreter → backtest-runner →
   metrics-computer → overfit-checker → report-writer) and merge.
...
| Path | Contents |
|---|---|
| `agent.md` | The backtest agent definition (main deliverable). |
| `subagents/` | Spec-interpreter / backtest-runner / metrics-computer / overfit-checker / report-writer. |
| `skills/` | 10 verbatim `SKILL.md` copies. See `skills-index.md`. |
| `workflows/` | 4 verbatim workflow `.md` files. See `workflows-index.md`. |
| `mcpservers/` | JSON entries: recommended backtest MCPs + installed on this machine. |
