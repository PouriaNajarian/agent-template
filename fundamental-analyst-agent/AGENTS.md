# AGENTS.md — fundamental-analyst-agent
...
A **self-contained fundamental-analyst agent package**: the agent definition
...
recommended for fundamental analysis — plus a full live inventory of the MCP
...
3. Load the specific skills the analysis needs from `skills/`
   (see `skills-index.md`).
4. Use the MCP tools listed in `agent.md`; confirm they exist via
   `mcp-inventory/system-mcps.md` / `mcp-inventory/mcp-functions.md`.
5. Fan out to `subagents/` (statement-reader → quality-auditor →
   valuation-engine → peer-comparer → thesis-writer) and merge.
...
| Path | Contents |
|---|---|
| `agent.md` | The fundamental-analyst agent definition (main deliverable). |
| `subagents/` | Statement-reader / quality-auditor / valuation-engine / peer-comparer / thesis-writer. |
| `skills/` | 12 verbatim `SKILL.md` copies. See `skills-index.md`. |
| `workflows/` | 4 verbatim workflow `.md` files. See `workflows-index.md`. |
| `mcpservers/` | JSON entries: recommended fundamentals MCPs + installed on this machine. |
