...
Recommended workflow: **`data-pipeline-build`**.

## What this repo is

A **self-contained data-engineer agent package**: the agent definition
...
recommended for data engineering — plus a full live inventory of the MCP
...
2. Load the workflow `workflows/data-pipeline-build.md`
   (+ `workflows/data-warehouse-build.md`, `workflows/fw-ml-data-pipeline.md`).
...
5. Fan out to `subagents/` for parallel work (builder, schema, quality,
   backfill, docs) and merge.
...
| Path | Contents |
|---|---|
| `agent.md` | The data-engineer agent definition (main deliverable). |
...
| `mcpservers/` | JSON entries: recommended data MCPs + installed on this machine. |
