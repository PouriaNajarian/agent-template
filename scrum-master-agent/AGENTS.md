...
Recommended workflow: **`fw-role-separated-agents`** (scrum cadence).

## What this repo is

A **self-contained scrum-master agent package**: the agent definition
...
recommended for scrum facilitation — plus a full live inventory of the MCP
...
2. Load the workflow `workflows/fw-role-separated-agents.md`.
3. Load the specific skills the ceremony needs from `skills/`
   (see `skills-index.md`).
4. Use the MCP tools listed in `agent.md`; confirm they exist via
   `mcp-inventory/system-mcps.md` / `mcp-inventory/mcp-functions.md`.
5. Fan out to `subagents/` (backlog-groomer → sprint-planner →
   standup-runner → retro-facilitator → metrics-reporter) and merge.
...
| Path | Contents |
|---|---|
| `agent.md` | The scrum-master agent definition (main deliverable). |
| `subagents/` | Backlog-groomer / sprint-planner / standup-runner / retro-facilitator / metrics-reporter. |
| `skills/` | 15 verbatim `SKILL.md` copies. See `skills-index.md`. |
| `workflows/` | 6 verbatim workflow `.md` files. See `workflows-index.md`. |
| `mcpservers/` | JSON entries: recommended agile MCPs + installed on this machine. |
