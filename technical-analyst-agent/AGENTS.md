# AGENTS.md — technical-analyst-agent
...
A **self-contained technical-analyst agent package**: the agent definition
...
recommended for technical analysis — plus a full live inventory of the MCP
...
5. Fan out to `subagents/` (data-loader → trend-scanner → indicator-reader →
   levels-engine → bias-writer) and merge.
...
| Path | Contents |
|---|---|
| `agent.md` | The technical-analyst agent definition (main deliverable). |
...
| `mcpservers/` | JSON entries: recommended TA MCPs + installed on this machine. |
