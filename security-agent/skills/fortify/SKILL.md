---
name: fortify
description: Universal security hardening and authorized red-team testing for ANY project (web apps, REST/GraphQL APIs, mobile, desktop). Use when the user wants to audit, harden, "make secure", pentest, threat-model, or security-review a codebase; asks "what is exposed/accessible", "is this safe to ship", or wants a security level from basic to maximum. Researches attack classes, hardens the code (report-first, fix-on-approval), then verifies with a parallel swarm of authorized attacker agents. Localhost/ow
---
